...
const allDayStringsInMonth = (d: Date) => {
...
export default function Monthly() {
  ...
  const [anchor, setAnchor] = useState(() => new Date()); // เดือนอ้างอิง
  ...
  /** ดึงข้อมูลทั้งเดือน: รองรับทั้ง date เป็นสตริง YYYY-MM-DD และเป็น ISO */
  async function loadMonth(uid: string, a: Date) {
    try {
      setLoading(true);

      const s = startOfMonth(a);
      const e = endOfMonth(a);
      const dayStrings = allDayStringsInMonth(a);
      ...
      // 1) date == any of dayStrings
      const listA = await fetchAllPages([
        Query.equal("userId", uid),
        Query.equal("date", dayStrings),
      ]);

      // 2) date between month range (ISO)
      const listB = await fetchAllPages([
        Query.equal("userId", uid),
        Query.between("date", s.toISOString(), e.toISOString()),
      ]);

      // รวม + ตัดซ้ำ + เรียง (ล่าสุดก่อน)
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await databases.updateDocument(DB_ID, TX_COLLECTION_ID, editDraft.$id, {
        ...
        category: editDraft.category ? editDraft.category.toString() : undefined, // ป้องกัน null
        date: editDraft.date, // คงวันเดิม
      });
      closeEdit();
      if (userId) loadMonth(userId, anchor);
    } catch (e) {
  ...
  return (
    <View style={s.wrap}>
      ...
      <View style={s.tabsRow}>
        ...
        <TabBtn label="Monthly" active onPress={() => {}} />
      </View>
      ...
      {/* List of all transactions in month */}
      <FlatList
        data={rows}
        keyExtractor={(it) => it.$id}
        refreshing={loading}
        onRefresh={() => userId && loadMonth(userId, anchor)}
